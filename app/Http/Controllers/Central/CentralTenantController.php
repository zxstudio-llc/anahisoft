...
class CentralTenantController extends Controller
{
    ...
    public function index()
    {
        ...
        return Inertia::render('app/tenants/index', [
            'tenants' => $tenantsData,
            'stats' => $stats,
    ...
    public function updateSubscription(Request $request, string $id)
    {
        ...
        try {
            ...
            $tenant->subscription_plan_id = $request->subscription_plan_id;
            ...
            if ($tenant->subscriptionPlan) {
                $tenant->run(function () use ($tenant, $oldPlanId, $oldActive) {
                    $invoiceUsage = \App\Models\Tenant\InvoiceUsage::first();

                    // Si cambió el plan o se activó la suscripción
                    if ($oldPlanId != $tenant->subscription_plan_id || (!$oldActive && $tenant->subscription_active)) {
                        if ($invoiceUsage) {
                        ...
                        // Si se activó la suscripción, registrar en la tabla de suscripciones
                        if (!$oldActive && $tenant->subscription_active) {
                            \App\Models\Tenant\Subscription::create([
                                'name' => $tenant->subscriptionPlan->name,
                                'plan_type' => $tenant->subscriptionPlan->billing_period,
    ...
    public function showSubscription(string $id)
    {
        ...
        return Inertia::render('app/tenants/manage-subscription', [
            'tenant' => [
                'id' => $tenant->id,
                'company_name' => $companyName,
                'primary_domain' => $primaryDomain ? $primaryDomain->domain : null,
