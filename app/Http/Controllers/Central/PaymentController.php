...
class PaymentController extends Controller
{
    ...
    public function toggleActive(Request $request, string $id)
    {
        try {
            ...
            $status = $tenant->is_active ? 'activado' : 'bloqueado';
            
            return response()->json([
                'success' => true,
                'message' => "Inquilino {$status} correctamente",
                'tenant' => [
                    'id' => $tenant->id,
            ...
            return response()->json([
                'success' => false,
                'message' => 'Error al cambiar el estado del inquilino: ' . $e->getMessage()
            ], 500);
        }
    }
